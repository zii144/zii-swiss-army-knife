...
import { quotedPrintableEncode, quotedPrintableDecode } from '@zii/text';
...
const L = {
  en: { title: 'Quoted-printable', desc: 'Encode or decode quoted-printable (RFC 2045).', mode: 'Mode', encode: 'Encode', decode: 'Decode', input: 'Input', copy: 'Copy', copied: 'Copied' },
  'zh-TW': { title: 'Quoted-printable', desc: '編碼或解碼 quoted-printable（RFC 2045）。', mode: '模式', encode: '編碼', decode: '解碼', input: '輸入', copy: '複製', copied: '已複製' },
};

export default function QuotedPrintableTool({ onBack, lang, backLabel, offlineLabel }: ToolViewProps) {
  const t = tr(L, lang);
  const [mode, setMode] = useState<Mode>('encode');
  const [input, setInput] = useState('Hello café');
  const [copied, setCopied] = useState(false);
  const output = mode === 'encode' ? quotedPrintableEncode(input) : quotedPrintableDecode(input);
