...
const L = { en: { title: 'Canada CPP', desc: 'Estimate employee CPP + CPP2 contributions (2026 YMPE/YAMPE). On-device.', gross: 'Annual gross (CAD)', cpp: 'CPP', cpp2: 'CPP2', total: 'Total' } };
const money = (n: number) => n.toLocaleString('en-CA', { style: 'currency', currency: 'CAD', maximumFractionDigits: 0 });

export default function Tool({ onBack, lang, backLabel, offlineLabel }: ToolViewProps) {
  const t = tr(L, lang);
  const [gross, setGross] = useState(80_000);
  const r = caCppEmployee(gross);
  return (
