...
const L = { en: { title: 'Canada GST / HST', desc: 'Add or extract GST/HST/PST combined rate by province. On-device.', amount: 'Amount (CAD)', province: 'Province', inclusive: 'Amount includes tax', net: 'Net', tax: 'Tax', gross: 'Gross' } };
...
export default function Tool({ onBack, lang, backLabel, offlineLabel }: ToolViewProps) {
  ...
  const r = caGstHst(amount, province, inclusive);
  return (
    <ToolPage title={t.title} description={t.desc} onBack={onBack} backLabel={backLabel} offlineLabel={offlineLabel}>
      <div className="tool__inline">
        ...
        <label className="tool__field"><span>{t.province}</span><Select value={province} onChange={(v) => setProvince(v as CaProvince)} options={PROVINCES.map((p) => ({ value: p, label: `${p} (${(CA_2026.gstHst[p] * 100).toFixed(2)}%)` }))} ariaLabel={t.province} /></label>
      </div>
