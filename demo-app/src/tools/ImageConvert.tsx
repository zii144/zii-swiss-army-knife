...
import { convertImage, type ImageFormat } from '../lib/compute';
...
export default function ImageConvert({ onBack, lang, backLabel, offlineLabel }: ToolViewProps) {
  const t = tr(L, lang);
  ...
  const run = async () => {
    ...
    try {
      ...
      setResult(out);
      setPreviewUrl(
        URL.createObjectURL(new Blob([out as unknown as BlobPart], { type: MIME[to] })),
      );
    } catch (e) {
